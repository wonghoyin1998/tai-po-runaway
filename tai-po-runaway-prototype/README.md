...
- 角色入口：參加者、Hunter、工作人員／總控；Hunter 及工作人員分別需要密碼
...
- 快速任務訊息：可載入開局、籌碼、風箏、一番賞、二人影相、復活、轉場、最後階段及集合範本，修改後再發布
- 風箏答案回報：只在風箏任務開放時讓存活參加者提交；答案只會傳送及顯示給已登入的工作人員
...
Hunter 預設密碼為 `123456`。工作人員密碼不會在登入畫面顯示。兩者均可用環境變數更改：

```bash
STAFF_PASSWORD=你的工作人員密碼 HUNTER_PASSWORD=你的Hunter密碼 npm run dev
```
...
1. 風箏任務按時間自動開放，或由工作人員在任務控制手動開啟。
2. 存活參加者畫面會出現 `回報答案` 輸入框。
3. 每位參加者只可提交一次；提交後畫面顯示 `答案已提交`。
4. 答案不會放入參加者／Hunter 收到的 WebSocket state，只有已驗證的工作人員連線會收到。
5. 工作人員 Dashboard 可查看姓名、答案、提交時間，並標記 `正確` 或 `不正確`。
6. 工作人員可按 `清除／重交`，讓該參加者再次提交。
7. 公開事件紀錄只記錄「已提交」或處理結果，不會記錄答案文字。
